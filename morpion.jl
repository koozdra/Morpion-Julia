...
@inline function eval_dna(dna::Array{UInt16,1}, board, possible_moves)
  # board = initial_board()
  # possible_moves = initial_moves()
  # morpion = Morpion()
  moves = Move[]
...
function moves_and_points_hash(moves::Array{Move,1})
  board = zeros(Bool, 46 * 46 * 4)
  @inbounds for move in moves
    board[dna_index(move)] = true
  end
  hash(board)
end
...
function moves_and_points_hash_uint64(moves::Array{Move,1})
  board = bit_board_build()
  @inbounds for move in moves
    idx = dna_index(move)
    int_idx = cld(idx, 64)
    bit_idx = (idx - 1) % 64
    board[int_idx] |= (UInt64(1) << bit_idx)
  end
  hash(board)
end

function bit_board_build()
  n_bits = 46 * 46 * 4
  n_ints = cld(n_bits, 64)  # ceiling division to get number of UInt64s needed
  zeros(UInt64, n_ints)
end

function bit_board_set_move!(move::Move, bit_board::Array{UInt64,1})
  idx = dna_index(move)
  int_idx = cld(idx, 64)
  bit_idx = (idx - 1) % 64
  bit_board[int_idx] |= (UInt64(1) << bit_idx)
end

function bit_board_unset_move!(move::Move, bit_board::Array{UInt64,1})
  idx = dna_index(move)
  int_idx = cld(idx, 64)
  bit_idx = (idx - 1) % 64
  # Create a mask with all 1s except 0 at the bit we want to unset
  mask = ~(UInt64(1) << bit_idx)
  bit_board[int_idx] &= mask
end

function bit_board_hash(bit_board::Array{UInt64,1})
  hash(bit_board)
end
