...
#162 phQpD/h2cJABAih7m653U5vmqr9f7f/39b6d8/f57/uv
...
#166 IIMnTahsGFUcZm6pTKDfayUY/1q329Pu1af+fW1m/8u///u
#167 FEBMv5nUNjXLREvmOhmKzZDdn/9+axrvyc83X1Nb//9s7/s
#167 RSJIh84liiFRcOPTeIshl+9a/vRX9+1P8+//yr3Qefv//9g
...
#169 IiAjf1nSmo7NUI3kulmIm2G7Pv79Vdf5nPPTmnf9f93N/+2
#169 7EBET5ZlVgRpAJW6RkR43b4oGXe/2/1lz9/t453v6//if3/u

#167 FEBMv5nUNjXLRGuLDQnHZpu7V39v6Pt6Z7zy9uvpN/7kv/2
#170 FEBMv6lokkqOS7M5RPJOs/2+zl/cc78s8b4+O721a/vUn/98
#170 FEFMq7grY1JGooKxrhlfxd66n/79d0+vMmeNU/Vbb+9fr99bQ
...
#ERROR: BoundsError()
# in validate_line at C:\dimitri\Morpion-Julia\morpion.jl:251 (repeats 2 times)
#while loading C:\dimitri\Morpion-Julia\runner.jl, in expression starting on line
# 314

# ERROR: BoundsError()
#  in validate_line at C:\dimitri\Morpion-Julia\morpion.jl:250
#  in make_move at C:\dimitri\Morpion-Julia\morpion.jl:559
#  in random_completion at C:\dimitri\Morpion-Julia\morpion.jl:170
#  in end_search at C:\dimitri\Morpion-Julia\morpion.jl:781
#  in anonymous at no file:599
#  in include at boot.jl:245
#  in include_from_node1 at loading.jl:128
#  in process_options at client.jl:285
#  in _start at client.jl:354
...
# idea: write complete state to file_path
## index
## end_searched
## cache (maybe)
...
#searchy = Searchy(unpack_pack("7EBET5ZlVgRpAJW6RkR43b4oGXe/2/1lz9/t453v6//if3/u"))
# p = unpack_pack("kiQjjq5SCREWSWmi1kYvbXp0r9Vzn++z308h3TdRm/X93l3/6fLW")
# println(score(p))
# random_completion(morpion_evaluator(p))
#
# println(score(p))

# p = unpack_pack("kiQjjq5SCREWSWmi1kYvbXp0r9Vzn++z308h3TdRm/X93l3/6fLW")
# e = morpion_evaluator(p)
# for i = 1:25
# 	remove_loose_moves(e);
# end
# println(score(p))
# println(score(e.morpion))
# random_completion(e)
# println(score(e.morpion))
#
...
function morpion_from_hash(morpion_cache::Dict{Uint64,Morpion}, searchy::Searchy)
	...
	morpion_cache[searchy.points_hash] = unpack_pack(searchy.pack)
end
...
function modification_triple(morpion, visits)
	morpion_dna = generate_dna(morpion)
...
focus_inactive_reset = 10000
focus = focus_min
# the minimum score multiplier number to get to before resetting focus
focus_min_reset = 1
...
while true
	...
	function exploreMinReducer(a, b)
		min = 2
		t = 0.33
	...
	function rank(searchy, t)
		return searchy.score - (searchy.visits / (searchy.score * t))
	end
	...
	function exploitReducer (a,b)
		...
		if ra == rb
			if a.step_visited < b.step_visited
				return a
			end
	...
	# 	searchy = reduce (exploitReducer, values(index))
	# else
	# 	searchy = reduce (exploreBoundReducer, values(index))
	# end
	...
	#if step & 1 == 0
	#	eval_morpion = modification_triple(morpion, searchy.visits)
	#else
	...
	#eval_morpion = modification_triple(morpion, searchy.visits)
	if searchy.visits & 1 == 0
		eval_morpion = modification(morpion, searchy.visits / 2)
	else
		eval_morpion = modification_triple(morpion, searchy.visits)
	end

	# if searchy.visits <= searchy.score * 2
	# 	eval_morpion = modification_triple(morpion, searchy.visits)
	# else
		eval_morpion = modification(morpion, searchy.visits)
	# end
	...
	if score(eval_morpion) >= min_accept && !haskey(index,eval_hash) && !haskey(searched,eval_hash)
		...
		#if score(eval_morpion) >= min_visited_score
			searchy.visits = 0
			#searchy.visits = max(0, searchy.visits - 1)
			#searchy.visits = max(0, searchy.visits - (searchy.visits % ))
	...
	if max_score >= min_score_end_search &&
		end_search_activated &&
		!haskey(end_searched, searchy.points_hash) #&&
		#searchy.visits > searchy.score * 0.2
		...
		for new_morpion in end_search_found
			...
			if score(new_morpion) >= score(end_morpion) + index_accept &&
				!haskey(index,ph) &&
				!haskey(searched,ph)
	...
	time_inactivity = step - last_active_step

	if (time_inactivity > focus_inactive_reset && focus > focus_min_reset)
